...
We currently assign notes a "Not Helpful" status if the max (upper confidence bound) of their intercept is less than -0.05, in addition to the rules on the raw intercept values defined in the previous section. We also currently assign notes a "Helpful" status if the min (lower confidence bound) of their intercept is at least 0.35.
...
- Update the minimum LCB threshold for notes to become CRH to 0.35 from 0.32. This dramatically reduces the number of notes that become CRH due to this rule, and reduces status flips.
- Tweak the anti-note-flipping logic: increase the threshold of percent of notes that can flip before triggering a re-run to 0.09 from 0.15.
